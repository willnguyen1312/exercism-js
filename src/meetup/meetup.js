...
export const meetup = (year, month, desc, day) => {
  ...
  const dayOffsetStart =
    (DAYS.indexOf(day) - new Date(year, month - 1, 1).getDay() + DAYS.length) %
    DAYS.length;
  ...
  if (desc === "last")
    descLookup =
      4 * DAYS.length + dayOffsetStart < new Date(year, month, 0).getDate()
        ? "fifth"
        : "fourth";
  /**
  ...
  return new Date(
    year,
    month - 1,
    DAYS.length * DESCRIPTORS.indexOf(descLookup) + 1 + dayOffsetStart
  );
};
